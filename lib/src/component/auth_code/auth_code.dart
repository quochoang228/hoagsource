...
typedef BaseAuthCodeErrorBuilder = Widget Function(
  BuildContext context,
  String? errorText,
);

class BaseAuthCode extends StatefulWidget {
  ...
  final BaseAuthCodeErrorBuilder errorBuilder;
  ...
  @override
  BaseAuthCodeState createState() => BaseAuthCodeState();
}

class BaseAuthCodeState extends State<BaseAuthCode>
  ...
  @override
  void didUpdateWidget(BaseAuthCode oldWidget) {
        ...
        Padding(
          ...
          child: RepaintBoundary(
            child: Container(
              ...
              decoration: BaseShapeDecorationWithPremultipliedAlpha(
  ...
  Widget build(BuildContext context) {
    _effectiveBorderRadius = widget.borderRadius ??
        context.baseTheme?.authCodeTheme.properties.borderRadius ??
    ...
    _effectiveGap = widget.gap ??
        context.baseTheme?.authCodeTheme.properties.gap ??
        BaseTokenSizes.sizes.x4s;

    _effectiveHeight = widget.height ??
        context.baseTheme?.authCodeTheme.properties.height ??
        BaseTokenSizes.sizes.xl;

    _effectiveWidth = widget.width ??
        context.baseTheme?.authCodeTheme.properties.width ??
        BaseTokenSizes.sizes.lg;

    _effectiveSelectedBorderColor = widget.selectedBorderColor ??
        context.baseTheme?.authCodeTheme.colors.selectedBorderColor ??
        BaseTokenColors.light.piccolo;

    _effectiveActiveBorderColor = widget.activeBorderColor ??
        context.baseTheme?.authCodeTheme.colors.activeBorderColor ??
        BaseTokenColors.light.beerus;

    _effectiveInactiveBorderColor = widget.inactiveBorderColor ??
        context.baseTheme?.authCodeTheme.colors.inactiveBorderColor ??
        BaseTokenColors.light.beerus;

    _effectiveErrorBorderColor = widget.errorBorderColor ??
        context.baseTheme?.authCodeTheme.colors.errorBorderColor ??
        BaseTokenColors.light.chichi;

    _effectiveSelectedFillColor = widget.selectedFillColor ??
        context.baseTheme?.authCodeTheme.colors.selectedFillColor ??
        BaseTokenColors.light.goku;

    _effectiveActiveFillColor = widget.activeFillColor ??
        context.baseTheme?.authCodeTheme.colors.activeFillColor ??
        BaseTokenColors.light.goku;

    _effectiveInactiveFillColor = widget.inactiveFillColor ??
        context.baseTheme?.authCodeTheme.colors.inactiveFillColor ??
        BaseTokenColors.light.goku;

    _effectiveTextStyle =
        context.baseTheme?.authCodeTheme.properties.textStyle ??
            BaseTokenTypography.typography.body.text24;

    _effectiveErrorTextStyle =
        context.baseTheme?.authCodeTheme.properties.errorTextStyle ??
            BaseTokenTypography.typography.body.text12;

    _effectiveTextColor = context.baseTheme?.authCodeTheme.colors.textColor ??
        BaseTokenColors.light.textPrimary;

    _effectiveCursorColor = widget.authFieldCursorColor ??
        context.baseTheme?.authCodeTheme.colors.textColor ??
        BaseTokenColors.light.textPrimary;

    _animationDuration ??= widget.animationDuration ??
        context.baseTheme?.authCodeTheme.properties.animationDuration ??
        BaseTokenTransitions.transitions.defaultTransitionDuration;

    _animationCurve ??= widget.animationCurve ??
        context.baseTheme?.authCodeTheme.properties.animationCurve ??
        BaseTokenTransitions.transitions.defaultTransitionCurve;

    _peekDuration ??= widget.peekDuration ??
        context.baseTheme?.authCodeTheme.properties.peekDuration ??
        BaseTokenTransitions.transitions.defaultTransitionDuration;

    final double effectiveDisabledOpacityValue = widget.disabledOpacityValue ??
        context.baseTokenOpacities?.disabled ??
        BaseTokenOpacities.opacities.disabled;

    final Duration effectiveErrorAnimationDuration = widget
            .errorAnimationDuration ??
        context.baseTheme?.authCodeTheme.properties.errorAnimationDuration ??
        BaseTokenTransitions.transitions.defaultTransitionDuration;

    final Curve effectiveErrorAnimationCurve = widget.errorAnimationCurve ??
        context.baseTheme?.authCodeTheme.properties.errorAnimationCurve ??
        BaseTokenTransitions.transitions.defaultTransitionCurve;
