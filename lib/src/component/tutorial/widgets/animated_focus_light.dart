...
import '../_tutorial.dart';

class TutorialAnimatedFocusLight extends StatefulWidget {
  final List<TutorialTargetFocus> targets;
  final Function(TutorialTargetFocus)? focus;
  final FutureOr Function(TutorialTargetFocus)? clickTarget;
  final FutureOr Function(TutorialTargetFocus, TapDownDetails)?
      clickTargetWithTapPosition;
  final FutureOr Function(TutorialTargetFocus)? clickOverlay;
  ...
  const TutorialAnimatedFocusLight({
    Key? key,
  ...
  TutorialAnimatedFocusLightState createState() => pulseEnable
      ? AnimatedPulseFocusLightState()
      : TutorialAnimatedStaticFocusLightState();
}

abstract class TutorialAnimatedFocusLightState extends State<TutorialAnimatedFocusLight>
  ...
  late TutorialTargetFocus _targetFocus;
  Offset _positioned = const Offset(0.0, 0.0);
  TutorialTargetPosition? _targetPosition;
  ...
  void _runFocus() {
    ...
    TutorialTargetPosition? targetPosition;
    try {
  ...
  Widget _getLightPaint(TutorialTargetFocus targetFocus) {
  ...
  SizedBox _getSizedPainter(TutorialTargetFocus targetFocus) {
    return SizedBox(
    ...
    return shape == ShapeLightFocus.RRect
        ? TutorialRectClipper(
            ...
            target: _targetPosition ??
                TutorialTargetPosition(Size.zero, Offset.zero),
            radius: _targetFocus.radius ?? 0,
            borderSide: _targetFocus.borderSide,
          )
        : TutorialCircleClipper(
            _progressAnimated,
            _positioned,
            _sizeCircle,
  ...
  CustomPainter _getPainter(TutorialTargetFocus target) {
    if (target.shape == ShapeLightFocus.RRect) {
      return TutorialLightPaintRect(
        colorShadow: target.color ?? widget.colorShadow,
        progress: _progressAnimated,
        offset: _getPaddingFocus(),
        target:
            _targetPosition ?? TutorialTargetPosition(Size.zero, Offset.zero),
        radius: target.radius ?? 0,
        borderSide: target.borderSide,
        opacityShadow: widget.opacityShadow,
      );
    } else {
      return TutorialLightPaint(
        _progressAnimated,
        _positioned,
        _sizeCircle,
        colorShadow: target.color ?? widget.colorShadow,
...
class TutorialAnimatedStaticFocusLightState extends TutorialAnimatedFocusLightState {
...
class AnimatedPulseFocusLightState extends TutorialAnimatedFocusLightState {
