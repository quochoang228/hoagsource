...
typedef BaseTokenControlBuilder = Widget Function(
...
class BaseTokenControl extends StatefulWidget {
  ...
  /// be null.
  final BaseTokenControlBuilder? builder;
  ...
  @override
  State<BaseTokenControl> createState() => _BaseTokenControlState();
}

class _BaseTokenControlState extends State<BaseTokenControl> {
  ...
  bool get _canAnimateFocus => widget.showFocusEffect && _isEnabled && _isFocused && _hasPrimaryFocus;
  ...
  @override
  void didUpdateWidget(BaseTokenControl oldWidget) {
  ...
  Widget build(BuildContext context) {
    final double effectiveDisabledOpacityValue = widget.disabledOpacityValue ?? context.baseTokenOpacities?.disabled ?? BaseTokenOpacities.opacities.disabled;

    // Focus effect properties.
    final Color effectiveFocusEffectColor = widget.focusEffectColor ??
        context.baseTokenEffects?.controlFocusEffect.effectColor ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlFocusEffect.effectColor;
    ...
    final double effectiveFocusEffectExtent = widget.focusEffectExtent ??
        context.baseTokenEffects?.controlFocusEffect.effectExtent ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlFocusEffect.effectExtent;

    final Duration effectiveFocusEffectDuration = widget.focusEffectDuration ??
        context.baseTokenEffects?.controlFocusEffect.effectDuration ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlFocusEffect.effectDuration;

    final Curve effectiveFocusEffectCurve = widget.focusEffectCurve ??
        context.baseTokenEffects?.controlFocusEffect.effectCurve ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlFocusEffect.effectCurve;

    // Pulse effect properties.
    final Color effectivePulseEffectColor = widget.pulseEffectColor ??
        context.baseTokenEffects?.controlPulseEffect.effectColor ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlPulseEffect.effectColor!;

    final double effectivePulseEffectExtent = widget.pulseEffectExtent ??
        context.baseTokenEffects?.controlPulseEffect.effectExtent ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlPulseEffect.effectExtent!;

    final Duration effectivePulseEffectDuration = widget.pulseEffectDuration ??
        context.baseTokenEffects?.controlPulseEffect.effectDuration ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlPulseEffect.effectDuration;

    final Curve effectivePulseEffectCurve = widget.pulseEffectCurve ??
        context.baseTokenEffects?.controlPulseEffect.effectCurve ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlPulseEffect.effectCurve;

    // Scale effect properties.
    final double effectiveScaleEffectScalar = widget.scaleEffectScalar ??
        context.baseTokenEffects?.controlScaleEffect.effectScalar ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlScaleEffect.effectScalar!;

    final Duration effectiveScaleEffectDuration = widget.scaleEffectDuration ??
        context.baseTokenEffects?.controlScaleEffect.effectDuration ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlScaleEffect.effectDuration;

    final Curve effectiveScaleEffectCurve = widget.scaleEffectCurve ??
        context.baseTokenEffects?.controlScaleEffect.effectCurve ??
        BaseTokenEffectsTheme(tokens: BaseTokens.light).controlScaleEffect.effectCurve;
          ...
          child: FocusableActionDetector(
            ...
            child: GestureDetector(
              ...
              child: TouchTargetPadding(
                minSize: widget.ensureMinimalTouchTargetSize ? Size(widget.minTouchTargetSize, widget.minTouchTargetSize) : Size.zero,
                child: AbsorbPointer(
                    ...
                    child: AnimatedScale(
                      ...
                      child: CommonPulseEffect(
                        ...
                        child: AnimatedOpacity(
                          ...
                          duration: context.baseTokenTransitions?.defaultTransitionDuration ?? BaseTokenTransitions.transitions.defaultTransitionDuration,
                          curve: context.baseTokenTransitions?.defaultTransitionCurve ?? BaseTokenTransitions.transitions.defaultTransitionCurve,
                          child: CommonFocusEffect(
